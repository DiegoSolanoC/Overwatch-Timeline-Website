    ...
    async loadData() {
        try {
            ...
            const isGitHubPages = window.location.hostname.includes('github.io') || 
                                  ...
                                  (window.location.hostname !== 'localhost' && window.location.hostname !== '127.0.0.1' && !window.location.hostname.startsWith('192.168.') && !window.location.hostname.startsWith('10.') && window.location.protocol !== 'file:');
            
            // On GitHub Pages, always prioritize data/events.json over localStorage
            if (isGitHubPages) {
                console.log('DataModel: GitHub Pages detected - loading from data/events.json');
                try {
                    ...
                    if (eventsResponse.ok) {
                        ...
                        if (eventsData.events && Array.isArray(eventsData.events) && eventsData.events.length > 0) {
                            ...
                            console.log('DataModel: Loaded', this.events.length, 'events from data/events.json (GitHub Pages)');
                        } else {
                            this.events = [];
                ...
                // For localhost: Check localStorage first for events (user's saved events)
                ...
                if (savedEvents) {
                    ...
                    // No saved events, try to load from data/events.json (for initial setup)
                    try {
                        const eventsResponse = await fetch('data/events.json');
                        if (eventsResponse.ok) {
                            const eventsData = await eventsResponse.json();
