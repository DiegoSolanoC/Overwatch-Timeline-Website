    ...
    createTrain(routeData, isMultiStop = false, journeyProgress = 0) {
        ...
        const getTrainEndModel = (callback) => {
            if (trainEndModelCache) {
                ...
                gltfLoader.load('TrainEnd.glb', (gltf) => {
        ...
        const getTrainMiddleModel = (callback) => {
            if (trainMiddleModelCache) {
                ...
                gltfLoader.load('TrainMiddle.glb', (gltf) => {
                    const model = gltf.scene;
                    const cached = model.clone(); // Clone model (materials are cloned automatically)
    ...
    createSatellite(config) {
        ...
        const applySatelliteMaterial = (model) => {
            model.traverse((child) => {
                if (child.isMesh) {
                    child.material = new THREE.MeshPhongMaterial({
                        color: color,
                        emissive: type === 'ISS' ? 0x4a1a6b : type === 'MarsShip' ? 0x440000 : 0x004488,
                        emissiveIntensity: 0.3,
                        transparent: true,
                        opacity: 0.9,
                        shininess: 30
        ...
        if (satelliteModelCache && gltfLoader) {
            ...
            gltfLoader.load('Satellite.glb', (gltf) => {
                ...
                // Fallback to simple box
                const size = type === 'ISS' ? 0.015 : type === 'MarsShip' ? 0.010 : 0.006;
                const geometry = new THREE.BoxGeometry(size, size, size * 1.5);
                const material = new THREE.MeshPhongMaterial({
                    color: color,
                    emissive: type === 'ISS' ? 0x004488 : type === 'MarsShip' ? 0x442200 : 0x222222,
                    emissiveIntensity: 0.3,
                    transparent: true,
                    opacity: 0.9
                });
        ...
        const orbitColor = 0x9b59b6; // Start with purple
    ...
    updateSatellites() {
        ...
        satellites.forEach(satellite => {
            ...
            if (data.type === 'ISS' || data.type === 'MarsShip') {
                ...
                const nextAngle = data.angle + data.orbitSpeed;
