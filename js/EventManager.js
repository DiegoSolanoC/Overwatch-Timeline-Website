...
class EventManager {
    ...
    /**
     * Load events from localStorage or fetch from data/events.json
     */
    async loadEvents() {
        const isGitHubPages = this.isGitHubPages();
        
        // On GitHub Pages, always prioritize data/events.json over localStorage
        // (since users can't edit on GitHub Pages, file is source of truth)
        if (isGitHubPages) {
            console.log('EventManager: GitHub Pages detected - loading from data/events.json');
            try {
                ...
                if (response.ok) {
                    ...
                    if (data.events && Array.isArray(data.events) && data.events.length > 0) {
                        ...
                        console.log('EventManager: Loaded', this.events.length, 'events from data/events.json (GitHub Pages)');
                        console.log('EventManager: Event names:', this.events.map(e => e.name || (e.variants && e.variants[0]?.name) || 'Unnamed'));
                        
                        // Save to localStorage for future use (but file is still source of truth)
                        this.saveEvents();
                ...
                console.log('EventManager: Could not load from data/events.json:', error.message);
            }
        }
        
        // For localhost: Try to load from localStorage first (user's saved changes)
        ...
        if (savedEvents) {
            try {
                ...
                // Always check if data/events.json has more events (file might be updated)
                try {
                    const response = await fetch('data/events.json');
                    if (response.ok) {
                        const data = await response.json();
                        if (data.events && Array.isArray(data.events) && data.events.length > localStorageEvents.length) {
                            // File has more events, use file version
                            this.events = data.events;
                            console.log('EventManager: File has more events (' + data.events.length + ' vs ' + localStorageEvents.length + '), using file version');
                            this.saveEvents();
                            this.syncEventsToGlobe();
                            return;
                        }
                    ...
                    // If file fetch fails, continue with localStorage
                    console.log('EventManager: Could not check data/events.json, using localStorage');
                }
        ...
        // If no localStorage, try to load from data/events.json (for initial setup)
        try {
            const response = await fetch('data/events.json');
            if (response.ok) {
                const data = await response.json();
