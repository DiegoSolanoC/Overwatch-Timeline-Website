...
class EventManager {
    ...
    setupEventListeners() {
        ...
        // Check if we're on GitHub Pages (disable event management)
        const isGitHubPages = this.isGitHubPages();
        if (toggleBtn && isGitHubPages) {
            toggleBtn.style.opacity = '0.5';
            toggleBtn.style.cursor = 'not-allowed';
            toggleBtn.title = 'Event Management (Localhost Only)';
        }

        if (toggleBtn && panel) {
            toggleBtn.addEventListener('click', (e) => {
                // Prevent action on GitHub Pages
                if (isGitHubPages) {
                    e.preventDefault();
                    e.stopPropagation();
                    alert('Event Management is only available on localhost. This feature is disabled on GitHub Pages.');
                    return;
                }
